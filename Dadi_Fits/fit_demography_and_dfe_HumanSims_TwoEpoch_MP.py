...
class DemographicInference():
    ...
    def fitDemographicModelParser(self):
        ...
        parser.add_argument('--Lsyn', type=float, default=5323177, dest='Lsyn',
                           help='Set the length of the number of synonymous sites')
        return parser
    ...
    def main(self):
        ...
        logger.info('Synonymous Sequence Length is: '+str(Ls))
        ...
        for i in range(25):
            ...
            logger.info(
                'Beginning optimization with guess, {0}.'.format(p0))
            try:
                logger.info("Print all the parameters going into the optimization")
                logger.info("p0 "+str(p0))
                logger.info("nonsyn_data "+str(nonsyn_data))
                logger.info("theta_nonsyn "+str(theta_nonsyn))
                logger.info("lower_bound "+str(lower_bound))
                logger.info("upper_bound "+str(upper_bound))
                logger.info("best_params "+str(best_params))
                popt,ll = dadi.Inference.opt(p0, nonsyn_data, spectra.integrate, pts=None,
                          func_args=[DFE.PDFs.gamma, theta_nonsyn],
                          lower_bound=lower_bound, upper_bound=upper_bound,
                          verbose=len(best_params), maxeval=400, multinom=False)
                logger.info(
                ...
                logger.info("expected_sfs "+str(expected_sfs))
                poisson_ll_nonsyn = dadi.Inference.ll(model=expected_sfs, data=nonsyn_data)
                logger.info("poisson_ll_nonsyn "+str(poisson_ll_nonsyn))
